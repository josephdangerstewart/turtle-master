...
for i, v in pairs( fs.list( "custom-elements" ) ) do
	cui.elements[v:sub(1, #v-4)] = dofile("custom-elements/" .. v)
end
...
function cui.removePanel(panel)
	for i = 1, #cui.roots do
		if cui.roots[i] == panel then
			table.remove(cui.roots, i)
		end
	end
end
