...
function panel.new( data, parent, isroot )
	...
	if not self.isroot and not self.detatched then self.parent = parent end
	self.children = { }
	...
	if self.parent and self.parent.sortChildren then
		for k, v in pairs( self.parent.children ) do
			if v == self then
				...
				table.insert( self.parent.children, v )
			end
		end
	else
...
function panel:mousepressed( x, y, button )
	...
	if self.state == cobalt.state or self.state == "_ALL" then
		...
		if self.freezeBackground == true then
			return true
		end
	end
end
